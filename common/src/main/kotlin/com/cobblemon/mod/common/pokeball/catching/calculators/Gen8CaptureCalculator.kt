...
object Gen8CaptureCalculator : CaptureCalculator, CriticalCaptureProvider {
    ...
    override fun processCapture(thrower: LivingEntity, pokeBallEntity: EmptyPokeBallEntity, target: PokemonEntity): CaptureContext {
        ...
        val bonusWorld = if (pokemon.level < 20) max((30 - pokemon.level) / 10, 1) else 1
        ...
        val modifiedCatchRate = (pokeBall.catchRateModifier.behavior(thrower, pokemon).mutator((3F * pokemon.maxHealth - 2F * pokemon.currentHealth) * catchRate, ballBonus) / (3F * pokemon.maxHealth)) * bonusStatus * bonusWorld
