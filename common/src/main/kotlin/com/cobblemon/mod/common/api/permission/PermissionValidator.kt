...
interface PermissionValidator {
     ...
     */
    fun hasPermission(player: ServerPlayer, permission: String, level: Int): Boolean = hasPermission(player, CobblemonPermission(permission, PermissionWorld.byNumericValue(level)))
     ...
     * @param level The permission level being queried. 4 is generally used for cheats.
     */
    fun hasPermission(source: CommandSourceStack, permission: String, level: Int): Boolean = hasPermission(source, CobblemonPermission(permission, PermissionWorld.byNumericValue(level)))
}